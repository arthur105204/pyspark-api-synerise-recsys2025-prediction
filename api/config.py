...
DEFAULT_MODEL_METADATA_PATH = "data/serving/model_metadata/baseline_lr_v1.json"
...
class ApiSettings:
    ...
    model_metadata_path: str = DEFAULT_MODEL_METADATA_PATH
    ...
    def resolved_model_metadata_path(self) -> Path:
        path = Path(self.model_metadata_path)
        if path.is_absolute():
            return path
        return PROJECT_ROOT / path
...
def get_settings() -> ApiSettings:
    return ApiSettings(
        ...
        model_metadata_path=os.getenv("MODEL_METADATA_PATH", DEFAULT_MODEL_METADATA_PATH),
