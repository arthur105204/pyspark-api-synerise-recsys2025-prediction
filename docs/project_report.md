...
## API Serving Plan Summary

Objective:
Create a lightweight lookup API for purchase propensity scores generated by the batch scoring pipeline.

Planned serving architecture:
The API should serve exported batch scores from SQLite. It should not load Spark or run Spark ML model inference per request.

Input scoring table:
`data/processed/scoring/purchase_propensity_scores/`

Planned serving export format:
...
Testing strategy:
API and repository tests should use a temporary SQLite database with fake client IDs only. Tests should cover health, metadata, successful score lookup, missing score lookup, score bounds, binary prediction labels, and missing database handling.

Privacy and security checks:
API examples should use fake client IDs only. The generated SQLite database should be ignored by git. The API should not expose raw features, labels, query text, product names, model binaries, or row-level examples from real data.

Current result:
Phase 7 is planned next. The full SQLite export should run in WSL/Linux or another Spark runtime that can read the local score Parquet output.

Limitations:
This is a local MVP lookup layer. It does not include external database deployment, authentication, caching, or load testing.

Next step:
Implement the serving export job and lookup API contract, then review API behavior before adding manual feature-input prediction and a demo UI.
...
- roadmap updated for API serving and demo interface
- Phase 7 split into lookup API, manual feature-input prediction, and demo UI
- Phase 8 separated as offline experiment tracking and tuning
- Phase 9 defined for load/stress testing, final report, and demo packaging
...
- FastAPI lookup app
- SQLite repository
- health, metadata, and score endpoints
- fake-data API tests
- API contract and serving artifacts
- full SQLite serving export
...
1. Implement the serving export job and lookup API contract.
2. Run the serving export job in WSL/Linux or a properly configured Spark runtime.
3. Review lookup API behavior against the generated SQLite database.
